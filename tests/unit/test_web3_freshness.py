...
def patch_deferred(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(w3mod, "deferred_response", _immediate_deferred_response)


def test_fresh_latest_callable_behavior() -> None:
    assert eth._fresh_latest("x", 10, 10) is True
    assert eth._fresh_latest("x", 10, 11) is True
    assert eth._fresh_latest("x", 10, 9) is False
...
def test_fresh_negative_requires_latest_callable_behavior() -> None:
    ...
    assert eth._fresh_negative_requires_latest({"ok": True}, 10, 0) is True
    assert eth._fresh_negative_requires_latest([], 10, 9) is True
    assert eth._fresh_negative_requires_latest("0xabc", 10, 1) is True

    # Negative result requires tip >= required_tip.
    assert eth._fresh_negative_requires_latest(None, 10, 9) is False
    assert eth._fresh_negative_requires_latest(None, 10, 10) is True
    assert eth._fresh_negative_requires_latest(None, 10, 11) is True
...
def test_web3_make_request_passes_freshness_through(patch_deferred: None) -> None:
    ...
    assert out == "0x2a"
    assert fp.calls[-1].freshness is eth._fresh_latest
...
def test_eth_get_balance_latest_uses_fresh_latest(patch_deferred: None) -> None:
    ...
    assert call.freshness is eth._fresh_latest
...
def test_eth_get_transaction_by_hash_uses_negative_requires_latest(patch_deferred: None) -> None:
    ...
    assert call.freshness is eth._fresh_negative_requires_latest
