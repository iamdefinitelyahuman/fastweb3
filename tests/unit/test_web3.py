...
class RecordingProvider(Provider):
    ...
    def request(
        ...
        *,
        ...
        # web3.Web3.make_request should always pass formatter=None into provider
        with self._lock:
            self._calls.append(Call(method, list(params), kind, formatter))
...
def force(x: Any) -> Any:
    """
    Force evaluation of the lazy Proxy returned by deferred_response.
    Identity is fine for primitives; for lists/dicts just touch them.
    """
    # Most of your methods return bool/int/str/list/dict. Equality checks force evaluation.
    _ = str(x)
    return x
...
# ----------------------------
# wiring + formatting: no-param methods
# ----------------------------


@pytest.mark.parametrize(
    "fn_name, rpc_method, raw, expected",
    [
        ("protocol_version", "eth_protocolVersion", "0x41", "0x41"),
        ("coinbase", "eth_coinbase", "0x" + "aa" * 20, "0x" + "aa" * 20),
        ("mining", "eth_mining", True, True),
        ("accounts", "eth_accounts", ["0x" + "bb" * 20], ["0x" + "bb" * 20]),
        # to_int formatters
        ("chain_id", "eth_chainId", "0x1", 1),
        ("hashrate", "eth_hashrate", "0x2a", 42),
        ("gas_price", "eth_gasPrice", "0x3b9aca00", 1_000_000_000),
        ("block_number", "eth_blockNumber", "0x10", 16),
    ],
)
def test_eth_no_param_methods_wiring_and_formatting(fn_name, rpc_method, raw, expected):
    p = RecordingProvider(results={rpc_method: raw})
    w3 = mk_w3(p)
    eth = w3.eth

    out = force(getattr(eth, fn_name)())
    assert out == expected
    ...
    assert c.params == []
    assert c.kind == "read"
    assert c.formatter is None  # make_request passes formatter=None to provider


def test_eth_syncing_normalizes_output():
    raw = {
        "startingBlock": "0x1",
        "currentBlock": "0x2",
        "highestBlock": "0x3",
    ...
    # normalize_rpc_obj converts known hex quantity fields to ints.
    # (These specific keys are NOT in HEX_QUANTITY_FIELDS; syncing result is client-dependent.)
    # So we only assert it returns a dict unchanged in shape.
...
def test_get_code_wiring():
    ...
    assert c.params == ["0x" + "ab" * 20, "0x7b"]  # 123 -> hex quantity
    assert c.kind == "read"
...
# ----------------------------
# write-kind methods and tx-object validation
# ----------------------------


def test_sign_is_write_kind_and_validates_data_hex():
    a = "0x" + "12" * 20
...
def test_send_raw_transaction_is_write_kind_and_disallows_empty():
...
def test_tx_object_fee_field_invariants_enforced():
    ...
    # gasPrice together with maxFeePerGas is invalid
    ...
    # only one of 1559 fields is invalid
...
def test_send_transaction_wiring_and_write_kind():
    p = RecordingProvider(results={"eth_sendTransaction": "0x" + "aa" * 32})
    w3 = mk_w3(p, strict=True)
...
def test_new_filter_builds_filter_object_and_wiring():
...
def test_simple_filter_creators_wiring(fn_name, rpc_method, raw):
    p = RecordingProvider(results={rpc_method: raw})
    w3 = mk_w3(p, strict=True)
...
def test_uninstall_filter_wiring():
...
def test_get_filter_changes_and_logs_normalize():
...
def test_get_logs_builds_filter_object_and_normalizes():
...
# ----------------------------
# non-strict mode: passthrough behavior sanity
# ----------------------------


def test_non_strict_does_not_lowercase_or_validate_str_address():
    # In non-strict mode, normalize_address returns stripped string unchanged.
